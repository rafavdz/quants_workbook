# Multivariate linear model{#Lab7}
...
We will follow-up the example we used before to introduce the simple linear model to illustrate the advantages of using more than one variable. To do so, please set your RStudio environment as follows:
...
The first thing to note is that both females and males go in the same direction. When the respondent's age increases the age of the partner increases as well. An interesting thing is that females tend to be located higher with respect to the Y axis compared to the male dots.

Therefore, we can imagine that not only the age of the respondent is involved in the decision on how people chooses their partner, but also sex. We can draw two lines one for male and the other for female respondents, instead of only a general one as we did in the previous lab. To do this we will use the `ggplot` function in combination with the `geom_parallel_slopes` function from the `moderndive` package:
...
ggplot(nilt_sample,  aes(x = rage, y = spage, color = rsex)) + 
  ...
  labs(title = "Respondent's age vs respondent’s spouse/partner age", 
       x = "Respondent's age", y = "Respondent’s spouse/partner age" )
```
Well, our suspicions that points representing female respondents were above the ones for males are turning out to be true. What we have in the plot above are the optimal parallel lines that describe our points the best for each group. The interpretation of our visualizations so far is: 
...
From the output, we see that there is one intercept $\beta_0$, one slope coefficient $\beta_1$ for the numeric variable `rage` and other coefficient $\beta_2$ for the categorical variable. If you observe closer, the output appended only one of the two categories for `rsex`. This is because categorical variables take one of the categories as the reference. The variable that it is not shown is the reference, 'Male' in this case.
...
Yes, it improved! Before it was about 1,175.
...
In the previous section, we used a small random sample of 40 people only. Now, let's fit a multivariate linear model using all the observations in the `nilt` dataset. In addition to the respondent age `rage` and `rsex`, I am interested to know whether the type of place where they live (e.g. big city, village, farm/country, etc.) `placeliv` plays a role in how people choose their partner.

First, I need to coerce the `placeliv` variable as factor. And I will have a quick look at a table of this variable to have an idea of how respondents are distributed and see how the factor levels are ordered.
```{r}
# coerce to factor
...
We see that most of the respondents live in 'a small city or town'. Also, note that the category 'Dont know' contains 0 observations. I will use the function `droplevels()` to remove the unused category in our variables. This also means that the first level in this variable will be 'a big city'. Then, I will fit the model and store it in an object called `m2`.
```{r}
...
1. We confirm the positive relationship between the respondent's age and his/her partner by a factor of 0.94. 
2. The difference in years by sex now is smaller. We see that the age of a female respondent partner is expected to be 3.8 years older compared to males.
...
The expected age of this partner is, therefore, 41.7 years old.
...
2. Save the Rmd document as 'Multivariate_lab_7'.
