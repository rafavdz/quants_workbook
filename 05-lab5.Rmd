...
When conducting empirical research we are often interested in associations between two variables, e.g. personal income and attitudes towards migrants. In this lab we will focus on visualizing relationships among variables and how to measure them. In quantitative research, the main variable of interest in an analysis is called the _dependent_ or _response variable_, and the second is known as the _independent_ or _explanatory_. In the example here, we can think of personal income as the independent variable and attitudes as the dependent.
...
4. Save the Rmd document as 'Lab5_correlation'.
...
To illustrate this type of correlation, let's start with a quite obvious but useful example. Suppose we are interested in how  people choose their spouse or parter. The first characteristic that we might look at is age. We might suspect that there is a correlation between the the `nilt` respondents' age and their partner's age. Since both ages are a numeric variables, a scatter plot is appropriate to visualize the correlation. To do this, let's use the functions `ggplot()` and `geom_point()`. In aesthetics `aes()` let's define the respondent's age `rage` in the X axis and the respondent's spouse/partner age `spage` in the Y axis. As a general convention in quantitative research, the response/dependent variable is visualized on the Y axis and the independent on the X axis (you do not need to copy and reproduce the example below).
...
From the plot above, we see that there is a positive strong correlation between the respondent's age and their partner's age. We see that for some individuals their partner's age is older, whereas others is younger. Also, there are some dots that are far away from the straight line. For example, in one case the respondent is around 60 years old and the age of the partner is around 30 years old (can you find that dot on the plot?). These extreme values are known as _outliers_.

We may also suspect that gender is playing a role in this relationship. We can include this as a third variable in the plot by colouring the dots by gender. To do this, let's specify the `colour` argument in aesthetics `aes()` with a categorical variable `rsex`.
```{r}
ggplot(nilt, aes(x = rage, y = spage, colour = rsex)) + 
  ...
  labs(title = "Respondent's age vs respondent’s spouse/partner age", 
       x = "Respondent's age", y = "Respondent’s spouse/partner age" )
```
In the previous plot, I included a line which describes what it would look like if the partner's age were exactly the same as the respondent's age. We observe a clear pattern in which female participants are on one side of the line and males on the other. Not too surprisingly, most female respondents tend to choose/have partners who are older, whereas males younger ones.
...
Let's say we are interested in the relationship between education level and perception of migration. The variable `highqual` contains the respondent's highest education qualification. Using `ggplot()`, I will situate `mig_per` on the X axis and `highqual` on the Y axis, and plot it with the `gem_boxplot()` function. Note that before passing the dataset to `ggplot`, I filtered out two categories of the variable `highqual` where education level is unknown (i.e. "Other, level unknown" or "Unclassified").
```{r}
nilt %>% 
  ...
  geom_boxplot()
```
From the plot above, we see that respondents with higher education level (on the bottom) have more positive opinion on migration when compared to respondents with lower education level or no qualifications (on the top). Overall, the lower the education level, the worse the opinion towards migration. Since education level is an ordinal variable, we can say this is a positive relationship.
...
* Do you think the opinion towards migration differs among the groups in the plot? Comment on the overall results in .Rmd document. 

## Measuring correlation

So far we have examined correlation by visualizing variables only. A useful practice in quantitative research is to actually measure the magnitude of the relationship between these variables. One common measure is the _Pearson_ correlation coefficient. This measure results in a number that goes from -1 to 1. A coefficient below 0 implies a negative correlation whereas a coefficient over 0 a positive one. When the coefficient is close to positive one (1) or negative (-1), it implies that the relationship is strong. By contrast, coefficients close to 0 indicate a weak relationship. This technique is appropriate to measure linear numeric relationships, that is when we have numeric variables with a normal distribution, e.g. age in our dataset. 
...
```
The coefficient is very close to 0, which means that the correlation is practically non-existent. The absence of correlation is also interesting in research. For instance, one could expect that younger people would be more open to migration. However, it seems that age does not play a role on people's opinion about migration in NI.
...
* Finally, knit the 'Lab5_correlation' Rmd document to `.html` or `.pdf`. The output document will automatically be saved in your project.
