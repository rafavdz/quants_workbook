...
`R` can handle many classes of data. It is crucial that you can distinguish the main. Broadly speaking there are two types of variables,

1. **categorical** and; 
2. **numeric** (formally know as interval or ratio). 

Categorical variables distinguished because they are limited in the number of categories it can take, e.g., country, name, political party, or gender. Ordinal data is a *sub-type* of the categorical, and it is used when the categories can be ranked and their order is meaningful, e.g., education level or level of satisfaction. Numeric values can be continuous (these are usually measured and can take infinite values, e.g. speed or time).^[For more details, please refer to the DataCamp module [Introduction to Data in R](https://learn.datacamp.com/courses/introduction-to-data-in-r).]
...
Now, let's get serious and explore Star Wars. Yes, the film series! The `starwars` data set from the `dplyr` package contains information about the characters, including height, hair colour, and sex (to get more information type `?starwars` in your console). At this time we will use a reduced version of the full data set.
