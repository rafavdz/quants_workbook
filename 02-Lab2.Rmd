...
In our previous lab we set up an RStudio Cloud session, we got familiar with the RStudio environment and some of the purpose and contents of its panes. In this Lab we will learn about R packages, how to install them and load them. Also, we will use different types of data. You will have the chance to practice with additional `R` operators. Lastly, we will load a real-world data set and put in practice your new skills.
...
1. Make sure you have a free [RStudio Cloud](https://rstudio.cloud/) account (in case you have not created one yet, please follow the guidance provided in Lab 1);
...
1. Once you have accessed the 'Lab 2' project, write or copy the following line in your script and run it:
```{r eval = FALSE}
...
3. Once the package is installed, you need to load it using the `library()` function as follows:
```{r eval = TRUE}
library(tidyverse)
```
...
There are couple of things you should. First, the packages need to be installed only per project in RStudio Cloud (and only once if you are working in RStudio Desktop version). By contrast, packages must be loaded using the `library()` function every time you restart an R session. 
...
`R` can handle many classes of data. It is crucial that you can distinguish the main since the type of data you are using must match the type of statistical analysis or visualization you produce.

Broadly speaking there are two types of variables, (1) **categorical** and (2) **numeric** (formally know as interval or ratio). The first is distinguished because it is limited in the number of categories it can take, e.g., country, name, political party, or gender. Ordinal data is a *sub-type* of the categorical, and it is used when the categories can be ranked and their order is meaningful, e.g., education level or level of satisfaction. Numerical values can be continuous (these are usually measured and can take infinite values, e.g. speed or time).^[For more details, please refer to the DataCamp module [Introduction to Data in R](https://learn.datacamp.com/courses/introduction-to-data-in-r).]
...
Now, let's get serious and explore Star Wars. Yes, the film series! The `starwars` data set from the `dplyr` package contains information about the characters, including height, hair and skin colour and sex (to get more information type `?starwars` in your console). At this time we will use a reduced version of the full data set.
...
glimpse(starwars2)
```
2. What do you observe this time? ... It seems that the data type is not consistent with their content. For example, the variables `species`, `gender`, and `hair_color` are of type `<chr>` (that is `character`), when according to what we just learnt they should be a factor. To transform them, we will use the function ´factor()´. This process is known as coercing a variable, that is when you change form one type to other.
...
```
6. Now it is telling us that this is a factor. So, this is what we want. Let's assign this coerced vector to our dataset.
```{r eval=FALSE}
starwars2$species <- factor(starwars2$species)
```
7. Let's check if the type of variable rally changed by glimpsing the data and checking the levels of `species`. 
```{r eval=FALSE}
glimpse(starwars2)
...
A useful operator is the pipe `%>%`. This is part of the `tidyverse` package. So, it is ready for you to use it. This operator passes the result of one operation to the next. Check the results of the following operations in your console:
...
If you look at your 'Environment' tab in pane 2, you will see there is a new object called `police`, which has 87 observations and 34 variables (or columns). To start exploring the contents, we will glimpse the `police` data as following:
```{r eval = FALSE}
glimpse(police)
```

As you can see, there are several variables included in the dataset, such as age, gender, law enforcement agency (`lawenforcementagency`), or whether the victim was armed (`armed`). You will see some of these variables are not in the adequate type. For instance, some are categorical and should be type `<fct>` instead of `<chr>`. 
...
Having a closer look to the reduced version, we can see that in fact all the variables are of type `<chr>`, including `age`.

Let's coerce the variables in to their correct type. We will start with age, from character to numeric:^[Age is not known for some cases. Thus, it is recorded as 'Unknown' in the dataset. Since this is not recognized as a numeric value in the coercion process, `R` automatically sets it as a missing value, `NA`.]
```{r eval = FALSE}
...
So, what did it just happen in the code above? First, the pipe operator, `%>%`: What we are doing verbally is _take the object `police_2`, THEN filter raceethnicity based on a condition_. Later, what is happening inside `filter`? Lets have a look at what R does in the background for us (Artwork by \@alison_horst):
...
In the example above, we are keeping the observations in `raceethnicity` that are NOT EQUAL to 'Unknown'. Finally, when we assigned the result to an object named as the same as our previous object. So, we replaced the _old_ dataset with the filtered version.
...
This is the end of Lab 2. Again, the changes in your script should be saved automatically in R Studio Cloud. However, make sure this is the case as you were taught in Lab 1. After this, you can close the tab in your web browser.
