...
This document presents the suggested R code to answer the workbook activities and exercises throughout the course labs in Quantitative Research Methods for Social Sciences. This covers from Lab 3 until Lab 9.
...
* Create a One-Way contingency table for `uninatid` using the `table()` function;
```{r}
table(nilt_subset$uninatid)
```
* Create a table showing the percentages for `uninatid` combining the functions `table()` and `prop.table()`;
```{r}
prop.table(table(nilt_subset$uninatid)) * 100
```
* Using the variables `religcat` and `uninatid`, generate a Two-Way contingency table and assign it to an object called `table2`;
```{r}
table2 <- table(nilt_subset$religcat, nilt_subset$uninatid)
```
* Print `table2` including the totals for rows and columns using the `addmargins()` function;
```{r}
addmargins(table2, c(1,2))
```
* Obtain the table of percentages of `table2` by columns using the function `prop.table()`.
```{r}
...
```
* Calculate the _mean_ and _median_ personal income `persinc2`, using `mean()` and `median()` function, respectively. Are they different? Why do you think so?;
```{r}
mean(nilt_subset$persinc2, na.rm = T)
median(nilt_subset$persinc2, na.rm = T)
```
* Calculate the quartiles for personal income `persinc2` using the `summary()` function. What do you observe?
```{r}
summary(nilt_subset$persinc2)
```
* Finally, compute the mean and standard deviation of the personal income `persinc2`, grouped by happiness `ruhappy`. What do you observe?... Even though we cannot answer the question above with this technique, it provides information about the association between personal income and happiness level.
```{r}
nilt_subset %>% 
  group_by(ruhappy) %>% 
  summarise(mean = mean(persinc2, na.rm = T), sd = sd(persinc2, na.rm = T))
```
...
* Knit the .Rmd document as HTML or PDF. The knitted file will be saved automatically in your project. You can come back to the Rmd file to make changes if needed and knit it again as many times as you wish. Remember: this is how you will be writing your research report (in R Markdown) and submit as a PDF. In fact, this suggested answer and all the previous lab workbooks are written in R Markdown, so do use them as examples for how may write and present your research report. 
